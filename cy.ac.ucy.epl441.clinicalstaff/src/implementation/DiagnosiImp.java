...
/**
 * This class implements the Diagnosi interface
 * and constructs the diagnosis page 
 * 
...
//@Reference
	 ...
	 * of the page and it's responsible to manage the users input
	 * 
	 * 
	...
	public void creatediagnosi(int patientsid) {
		...
		treatment.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				TreatmentRecordImp treatm = new TreatmentRecordImp(patientsid, treatmentservice, patientService,
						consultationservice);
		...
		consultation.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				ConsultationsImp cons = new ConsultationsImp(patientsid, treatmentservice, patientService,
						consultationservice);
		...
		patientinfo.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				PersonalInfoImp info = new PersonalInfoImp(patientsid, treatmentservice, patientService,
						consultationservice);
			...
			public void actionPerformed(ActionEvent e) {
				if (box.isSelected()) {
					treatmentservice.create(
							new Treatment(patientsid, "Use the previous Prescription", (java.sql.Date) new Date()));
					...
					treatmentservice.create(
							new Treatment(patientsid, drug + "," + drug2 + "," + drug3, (java.sql.Date) new Date()));
		...
		effects.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				for (i = 0; i < list.size(); i++) {
					...
					if (treatmentservice.get(trea).getDescription().contains(takedrug1)
							|| treatmentservice.get(trea).getDescription().contains(takedrug2)
